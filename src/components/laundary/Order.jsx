...
const App = () => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (data && Array.isArray(data.laundry)) {
          ...
          console.error('API response is not a valid object with a "laundry" array:', data);
          setOrders([]); 
        }
  ...
  const filteredOrders = orders
    .filter(order => {
      ...
      if (searchQuery && !order.requestedByName?.toLowerCase().includes(searchQuery.toLowerCase()) && !order._id?.toLowerCase().includes(searchQuery.toLowerCase())) {
        return false;
      }
      return true;
    })
  ...
  const OrderForm = ({ order, onSave, onClose }) => {
    const [formData, setFormData] = useState({
      ...
      items: order?.items || [{ itemName: '', quantity: 1, laundryServiceType: 'wash', itemType: '' }],
    ...
    const handleAddItem = () => {
      ...
      setFormData({ ...formData, items: [...formData.items, { itemName: '', quantity: 1, laundryServiceType: 'wash', itemType: '' }] });
    };
      ...
      <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-6 relative transform transition-all duration-300 scale-100 opacity-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="requestedByName" className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                <Package size={16} className="inline-block mr-1 text-green-500" /> Items
              </label>
...
>
  ...
  <option value="uniform">Uniform</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="laundryStatus" className="block text-sm font-medium text-gray-700 mb-1">
                  <List size={16} className="inline-block mr-1 text-purple-500" /> Status
                </label>
                <select
                  id="laundryStatus"
                  name="laundryStatus"
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'partially_delivered':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800"><Truck size={12} className="mr-1"/> Partial Delivery</span>;
      case 'cancelled':
                  ...
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {order.items?.map((item, index) => (
                        <div key={index}>{item.quantity}x {item.itemName} ({item.itemType})</div>
                      ))}
                    </td>
